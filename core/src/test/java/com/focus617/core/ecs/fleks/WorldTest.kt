...
class WorldTest {
    ...
    fun `cannot add the same system twice`() {
        assertThrows(FleksSystemAlreadyAddedException::class.java) {
            World {
    ...
    fun `cannot access a system that was not added`() {
        ...
        assertThrows(FleksNoSuchSystemException::class.java) {
            w.system<WorldTestIntervalSystem>()
        }
    }
    ...
    fun `cannot inject the same type twice`() {
        assertThrows(FleksInjectableAlreadyAddedException::class.java) {
            World {
                inject("42")
                inject("42")
            }
    ...
    fun `cannot add same ComponentListener twice`() {
        assertThrows(FleksComponentListenerAlreadyAddedException::class.java) {
            World {
                componentListener<WorldTestComponentListener>()
                componentListener<WorldTestComponentListener>()
            }
    ...
    fun `throw exception when there are unused injectables`() {
        assertThrows(FleksUnusedInjectablesException::class.java) {
            World {
                inject("42")
            }
        }
    ...
    fun `cannot use local classes as dependencies`() {
        ...
        assertThrows(FleksInjectableWithoutNameException::class.java) {
            World {
                inject(Local())
            }
        }
...
//    assertContentEquals(listOf(e1, e3), actualEntities)
        assertEquals(listOf(e1, e3), actualEntities)
    }
